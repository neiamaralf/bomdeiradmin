  ...
  hideKeyboard() {
    ...
    /*if (isAndroid) {
      try {
        ...
        let inputManager = Context.getSystemService(Context.INPUT_METHOD_SERVICE);
        inputManager.hideSoftInputFromWindow(activity.getCurrentFocus().getWindowToken(), app.android.view.inputmethod.InputMethodManager.HIDE_NOT_ALWAYS);
      } catch (err) {
        console.log(err);
      }
    }*/
  ...
  save() {
    this.db
      ...
      .subscribe(res => {
       ...
       /// if(this.page.focus())topmost().goBack();
