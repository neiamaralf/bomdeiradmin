...
export class BuscaCepComponent implements OnInit {
  ...
  onclick(item) {
    console.dir(item);
    this.routerExtensions.navigate(["/locais/"+"inserir/"+item.cep+"/"+item.logradouro+"/"+item.bairro+"/"+item.localidade+"/"+item.uf+"/"+this.params.get("idcategoria")+"/"+this.params.get("idadmin")], { clearHistory: false });
